...
TARGET_BOT = '@alexbypassbot'  # ⬅️ CHANGED TO ALEX BYPASS BOT
CACHE_CHANNEL_ID = -1003543194677  # ⬅️ CHANNEL TO CHECK FOR CACHED LINKS
TIMEOUT = 25  # ⬅️ INCREASED TO 25 SECONDS
...
def filter_bot_name(text):
    ...
    for bot_name in ['alexbypassbot', 'AlexBypass', 'alex_bypass_bot']:
...
def extract_bypass_link(text):
    """Helper to extract bypass link from response text"""
    ...
    if len(all_urls) >= 2:
        ...
        return response_original, bypass_url
    return None, None
...
# 1. Listener for Bypass Bot direct messages
def setup_userbot_handler(client):
    @client.on(events.NewMessage(from_users=TARGET_BOT))
    async def bypass_response_handler(event):
        response_text = filter_bot_name(event.text)
        
        response_original, bypass_url = extract_bypass_link(response_text)
        
        if not response_original or not bypass_url:
            return
        ...
        for link_id, request_data in list(active_requests.items()):
            ...
            if compare_links(stored_link, response_original):
                print(f"✅ MATCH FOUND (Direct Bot)! ID: {link_id}")
...
# 2. Background Catch Listener for the specific Channel
async def channel_catch_listener(client):
    """Background task to catch bypassed links from the channel"""
    @client.on(events.NewMessage(chats=CACHE_CHANNEL_ID))
    async def channel_handler(event):
        ...
        # Check if this matches any pending active requests
        for link_id, request_data in list(active_requests.items()):
            if request_data.get('matched'):
                continue
            ...
            if compare_links(stored_link, response_original):
                print(f"✅ MATCH FOUND (Channel Catch)! ID: {link_id}")
                request_data['matched'] = True
                request_data['future'].set_result({
                    'bypass_url': bypass_url,
                    'original_link': response_original
...
# 3. Function to check cache channel history for an existing link
async def check_cache_channel(client, original_link):
    """Scans recent messages in the cache channel for the original link"""
    try:
        # Scan last 100 messages for matching original link
        async for message in client.iter_messages(CACHE_CHANNEL_ID, limit=100):
            if message.text:
                text = filter_bot_name(message.text)
                response_original, bypass_url = extract_bypass_link(text)
                
                if response_original and bypass_url:
                    if compare_links(original_link, response_original):
                        print(f"🎯 CACHE HIT! Found in channel: {bypass_url}")
                        return bypass_url
    except Exception as e:
        print(f"⚠️ Error checking cache channel: {e}")
    return None
...
async def root():
    return {
        ...
        "version": "2.1",
        "description": "25s timeout, Cache check enabled, Channel Catch enabled",
        "endpoints": {
...
async def bypass_url(url: str, request: Request):
    """Bypass a shortened URL - 25s timeout, Cache Check, Instant Delete"""
    global user_client
    ...
    # ============================================
    # STEP 1: CHECK CACHE CHANNEL FIRST
    # ============================================
    print(f"🔍 Checking cache channel for: {formatted}")
    cached_bypass = await check_cache_channel(user_client, formatted)
    
    if cached_bypass:
        return {
            "success": True,
            "original_url": url,
            "bypassed_url": cached_bypass,
            "source": "cache"
        }

    # ============================================
    # STEP 2: IF NOT IN CACHE, SEND TO BOT
    # ============================================
    ...
    try:
        print(f"📤 Sending to {TARGET_BOT}: {formatted}")
        await user_client.send_message(TARGET_BOT, formatted)
        
        try:
            # Wait for response (25 seconds)
...
# ============================================
# ADMIN PANEL (Simplified for brevity, same logic as before)
# ============================================
    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .container { background: white; padding: 40px; border-radius: 20px; box-shadow: 0 20px 60px rgba(0,0,0,0.3); max-width: 450px; width: 100%; }
            h1 { text-align: center; color: #333; font-size: 28px; margin-bottom: 5px; }
            .subtitle { text-align: center; color: #888; margin-bottom: 30px; font-size: 14px; }
            .status-box { background: #f8f9fa; padding: 15px; border-radius: 12px; margin-bottom: 20px; text-align: center; }
            .status-badge { display: inline-block; padding: 6px 15px; border-radius: 20px; font-weight: 600; font-size: 14px; }
            ...
            .step { background: #f0f0ff; padding: 15px; border-radius: 10px; margin-bottom: 15px; border-left: 4px solid #667eea; }
            ...
            .phone-display { background: #e9ecef; padding: 8px 12px; border-radius: 6px; font-family: monospace; font-size: 14px; }
            .active-requests { margin-top: 15px; padding: 10px; background: #f8f9fa; border-radius: 8px; font-size: 13px; color: #666; }
            .timeout-info { margin-top: 10px; padding: 8px; background: #fff3cd; border-radius: 6px; font-size: 13px; color: #856404; text-align: center; }
        </style>
    </head>
    <body>
        <div class="container">
            ...
            <div id="userbotControls" style="display:none; margin-top:20px;">
                ...
                <div class="timeout-info">
                    ⏱️ Timeout: 25 seconds | Cache: ON | Catch: ON
                </div>
            </div>
            ...
            <div style="margin-top: 20px; padding: 15px; background: #f8f9fa; border-radius: 10px;">
                <p style="font-size: 14px; color: #666;">
                    ...
                    <code style="background: #e9ecef; padding: 4px 8px; border-radius: 4px;">/bypass?url=LINK</code>
                </p>
            ...
            <div class="footer">
                <p>Powered by Semy Bypass API v2.1 | Timeout: 25s | Cache & Catch</p>
            </div>
        </div>
        ...
        <script>
            ...
            window.onload = function() { checkStatus(); setInterval(checkStatus, 5000); };
            
            function showSection(section) {
                document.querySelectorAll('.section').forEach(el => el.classList.remove('active'));
                const sectionMap = { 'login': 'loginSection', 'otp': 'otpSection', 'password': 'passwordSection' };
                if (sectionMap[section]) document.getElementById(sectionMap[section]).classList.add('active');
            ...
            async function checkStatus() {
                try {
                    ...
                    if (data.connected) {
                        ...
                        if (data.phone) { phoneDisplay.style.display = 'inline'; phoneDisplay.textContent = '📱 ' + data.phone; }
                    ...
                    if (data.active_requests !== undefined) activeCount.textContent = data.active_requests;
                } catch (error) { console.error('Error checking status:', error); }
            }
            
            async function sendOTP() {
                const phone = document.getElementById('phoneInput').value.trim();
                if (!phone || phone.length < 10) { showMessage('Please enter a valid phone number', 'error'); return; }
                const btn = document.getElementById('sendOtpBtn');
                btn.disabled = true; btn.textContent = '⏳ Sending...'; hideMessages();
                try {
                    const response = await fetch('/admin/login/send-otp', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ phone: phone }) });
                    const data = await response.json();
                    if (data.success) {
                        loginSessionId = data.session_id; currentPhone = phone;
                        document.getElementById('otpPhoneDisplay').textContent = phone;
                        document.getElementById('otpInput').value = '';
                        showSection('otp'); document.getElementById('otpInput').focus();
            ...
            async function verifyOTP() {
                ...
                if (!otp || otp.length < 4) { showMessage('Please enter valid OTP', 'error'); return; }
                if (!loginSessionId) { showMessage('Please request OTP first', 'error'); return; }
                const btn = document.getElementById('verifyOtpBtn');
                btn.disabled = true; btn.textContent = '⏳ Verifying...'; hideMessages();
                try {
                    const response = await fetch('/admin/login/verify-otp', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ session_id: loginSessionId, otp: otp }) });
                    const data = await response.json();
                    if (data.success) {
                        if (data.requires_password) {
                            showSection('password'); document.getElementById('passwordInput').value = ''; document.getElementById('passwordInput').focus();
                            showMessage('🔐 Enter your 2FA password', 'info');
                        } else {
                            showMessage('✅ Login successful!', 'success');
                            setTimeout(() => { checkStatus(); showSection('login'); }, 1000);
                        }
                    } else { showMessage('❌ ' + data.message, 'error'); }
                } catch (error) { showMessage('❌ Network error', 'error'); }
            ...
            async function verifyPassword() {
                ...
                if (!password) { showMessage('Please enter your 2FA password', 'error'); return; }
                if (!loginSessionId) { showMessage('Session expired. Please try again.', 'error'); return; }
                const btn = event.target;
                btn.disabled = true; btn.textContent = '⏳ Verifying...'; hideMessages();
                try {
                    const response = await fetch('/admin/login/verify-password', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ session_id: loginSessionId, password: password }) });
                    const data = await response.json();
                    if (data.success) {
                        showMessage('✅ Login successful!', 'success');
                        setTimeout(() => { checkStatus(); showSection('login'); }, 1000);
            ...
            function resendOTP() {
                if (!currentPhone) { showMessage('Please enter phone number again', 'error'); return; }
                showSection('login'); setTimeout(() => { document.getElementById('sendOtpBtn').click(); }, 500);
            }
            
            function goBack() { showSection('login'); document.getElementById('phoneInput').value = currentPhone || '+91'; hideMessages(); }
            function goBackOtp() { showSection('otp'); document.getElementById('otpInput').focus(); hideMessages(); }
            ...
            function showMessage(msg, type) {
                ...
                div.textContent = msg; div.className = 'message ' + type; div.style.display = 'block';
                setTimeout(() => { div.style.display = 'none'; }, 6000);
            }
            
            function hideMessages() { document.getElementById('message').style.display = 'none'; }
            
            document.getElementById('phoneInput').addEventListener('keypress', function(e) { if (e.key === 'Enter') sendOTP(); });
            document.getElementById('otpInput').addEventListener('keypress', function(e) { if (e.key === 'Enter') verifyOTP(); });
...
async def send_otp(request: Request):
    try:
        ...
        if not phone: return {"success": False, "message": "Phone number required"}
        ...
        if not await client.is_user_authorized():
            ...
            return {"success": True, "session_id": session_id, "message": "OTP sent to your Telegram"}
        else:
...
async def verify_otp(request: Request):
    try:
        ...
        otp = data.get('otp')
        if not session_id or not otp: return {"success": False, "message": "Missing session_id or OTP"}
        if session_id not in login_clients: return {"success": False, "message": "Invalid session"}
        
        client = login_clients[session_id]
        try:
            await client.sign_in(code=otp)
            if not await client.is_user_authorized():
                return {"success": True, "requires_password": True, "message": "2FA password required"}
            ...
            # Setup direct listener and channel catch listener
            setup_userbot_handler(user_client)
            asyncio.create_task(channel_catch_listener(user_client))
            
            if session_id in login_clients: del login_clients[session_id]
...
async def verify_password(request: Request):
    try:
        ...
        if not session_id or not password: return {"success": False, "message": "Missing session_id or password"}
        if session_id not in login_clients: return {"success": False, "message": "Invalid session"}
...
async def admin_status():
    ...
    return {"connected": connected, "phone": phone, "active_requests": len(active_requests)}
...
async def lifespan(app: FastAPI):
    global user_client
    print("\n" + "="*50)
    print("🚀 STARTING SEMY BYPASS API v2.1")
    print("="*50)
    print(f"⏱️ Timeout: {TIMEOUT} seconds")
    print(f"📢 Cache Channel ID: {CACHE_CHANNEL_ID}")
